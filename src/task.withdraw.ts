    ...
    run: function(c: Creep) {
        ...
        if(c.memory.target){
            ...
            target = c.room.controller.pos.findClosestByRange(FIND_STRUCTURES, {
                filter: (s: Structure) => s.structureType === STRUCTURE_CONTAINER && s.store[RESOURCE_ENERGY] >= (c.carryCapacity -c.carry[RESOURCE_ENERGY])});
            if(target)
            c.memory.target = target.id;
        }
        else {
            target = c.pos.findClosestByRange(FIND_STRUCTURES, {
                filter: (s: Structure) => s.structureType === STRUCTURE_CONTAINER && !s.memory.transportTarget && s.store[RESOURCE_ENERGY] >= (c.carryCapacity -c.carry[RESOURCE_ENERGY])});
            if(target)
            c.memory.target = target.id;
        }
