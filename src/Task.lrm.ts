    ...
    run() {
        ...
        switch (this.c.memory.state) {
            ...
            case STATE_TARGETING:
                if (this.c.carry[RESOURCE_ENERGY] == this.c.carryCapacity) this.target('build');
                else this.target('mine');
                break;
            case STATE_MINING:
                this.mine();
                break;
    ...
    target(targetType: string) {
        switch (targetType) {
            case 'mine':
                ...
                break;
            case 'build':
                var site = this.c.pos.findInRange<ConstructionSite>(FIND_CONSTRUCTION_SITES, 2, { filter: (s: Structure) => s.structureType === STRUCTURE_CONTAINER })[0];
                if (!site) site = this.c.pos.findClosestByRange<ConstructionSite>(FIND_CONSTRUCTION_SITES)
                if (site) {
    ...
    mine() {
        ...
        switch (this.c.harvest(target)) {
            case ERR_NOT_IN_RANGE:
                ...
                break;
            case OK:
                var container = this.c.pos.findInRange<StructureContainer>(FIND_STRUCTURES, 2, { filter: (s: Structure) => s.structureType === STRUCTURE_CONTAINER })[0];
                if (this.c.carry[RESOURCE_ENERGY] >= this.c.carryCapacity * .75) {
                    if (container) this.c.transfer(container, RESOURCE_ENERGY);
                    else this.c.memory.state = STATE_TARGETING;
                    break;
                }
        }
